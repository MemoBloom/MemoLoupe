...
- GET 提供 output-dir 静态文件只读服务：``shot-analysis.html`` 与
  ...
  否则 400/403；
- 每次 GET ``/`` 或 ``/shot-analysis.html`` 前以 ``server_mode=True`` 重渲染
  （保证页面看到最新 corrections）；重渲染失败回退磁盘现有文件并记 stderr；
...
#: 只读静态服务允许的顶层目录/文件。
_ALLOWED_TOP = frozenset({"clips", "evidence", "raw", "shot-analysis.html"})
